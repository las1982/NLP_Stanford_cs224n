...
class NaiveBayes:
    ...
    def classify(self, words):
        ...
        for klass in self.classes:
            ...
            # print klass, len(self.documents[klass]), self.total, logprob
            for word in words:
                p_word = (self.countWord(klass, word) + 1) / (w + v)
                print self.countWord(klass, word), w, v
                logprob += math.log(p_word)
            results[klass] = logprob
        # print results['pos'], results['neg']
        return 'pos' if results['pos'] > results['neg'] else 'neg'

    def countWord(self, klass, word):
        # returns count word=word in klass=klass
        count = 0.0
    ...
    def countWords(self, klass):
        # retyrns count all words in klass=klass (not distinct)
        words = 0.0
        for document in self.documents[klass]:
    ...
    def countVocab(self, klass):
        # returns count all distinct words in klass=klass
        vocab = {}
        for document in self.documents[klass]:
            for word in document:
                vocab[word] = True
