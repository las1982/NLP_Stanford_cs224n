...
public class SpamLord {
    ...
    private String
            phoneSep = "(([-\\s])" + "|" + "(\\)\\s?))",
            phone3d1 = "(?<phone3d1>\\d{3})",
            phone3d2 = "(?<phone3d2>\\d{3})",
            phone4d = "(?<phone4d>\\d{4})";
    ...
    public List<Contact> processFile(String fileName, BufferedReader input) {
        ...
        try {
            for (String line = input.readLine(); line != null; line = input.readLine()) {
                m = eMailPt.matcher(line);
                p = phonePt.matcher(line);
                while (m.find()) {
//                    email = (m.group(1) + "@" + m.group(9) + m.group(13)).replace(" dot ", ".").replace(" dt ", ".");
                    email = m.group("name") + "@" + m.group("host") + m.group("domain");
                    email = email.replace(" dot ", ".").replace(" dt ", ".");
                    email = email.replace(" dt ", ".");
                    email = email.replace(";", ".");
                    contacts.add(new Contact(fileName, "e", email));
                }
                while (p.find()) {
                    phone = (p.group("phone3d1") + "-" + p.group("phone3d2") + "-" + p.group("phone4d"));
                    contacts.add(new Contact(fileName, "p", phone));
                }
            }
            input.close();
        } catch (IOException e) {
